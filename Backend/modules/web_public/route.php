...
function route_web_public(string $action, PDO $pdo): bool
{
    switch ($action) {
        case 'web_config_obtener':
        ...
        case 'web_features_listar':
            $stmt = $pdo->query("SELECT id, titulo, texto, orden, activo
                                 FROM site_features
                                 WHERE activo = 1
        ...
        case 'web_planes_listar':
            ...
            foreach ($planes as &$plan) {
                ...
                $plan['incluye'] = array_values(array_filter(
                    array_map(
        ...
        case 'web_testimonials_listar':
            $stmt = $pdo->query("SELECT id, nombre, rol, cita, orden, activo
        ...
        case 'web_home_obtener':
            ...
            $stmtFeatures = $pdo->query("SELECT id, titulo, texto, orden
                                         FROM site_features
                                         WHERE activo = 1
                                         ORDER BY orden ASC, id ASC");
            $features = $stmtFeatures->fetchAll();

            $stmtPlans = $pdo->query("SELECT 
                                        id, 
                                        nombre, 
                                        precio, 
                                        nota, 
            ...
            $stmtTestimonials = $pdo->query("SELECT 
                                                id, 
                                                nombre, 
                                                rol AS cargo, 
                                                cita AS texto, 
            ...
            $stmtMedia = $pdo->query("SELECT id, tipo, url, nombre_archivo, mime_type, tamanio_bytes
                                      FROM hero_media
                                      WHERE activo = 1
                                      ORDER BY id DESC
                                      LIMIT 1");
